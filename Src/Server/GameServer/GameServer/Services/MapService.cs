...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        private void OnMapTeleport(NetConnection<NetSession> sender, MapTeleportRequest message)
        {
            ...
            {
                Log.WarningFormat("Source TeleportID [{0}] not existed", message.teleporterId);
                return;
            }
            ...
            {
                Log.WarningFormat("Source TeleporterID [{0}] LinkTo ID [{1}] not existed", message.teleporterId, source.LinkTo);
            }
