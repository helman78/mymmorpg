...
namespace Services
{
    class UserService : Singleton<UserService>, IDisposable
    {
        ...
        public UserService()
        {
            ...
            MessageDistributer.Instance.Subscribe<UserCreateCharacterResponse>(this.OnCharCreate);
        ...
        public void Dispose()
        {
            ...
            MessageDistributer.Instance.Unsubscribe<UserCreateCharacterResponse>(this.OnCharCreate);
        ...
        {
            Debug.LogFormat("CharCreateRequest::charName:{0},charClass:{1}", charName, charClass.ToString());
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
        ...
        public void OnCharCreate(object sender, UserCreateCharacterResponse response)
        {
            Debug.LogFormat("OnCharCreate:{0} [{1}]", response.Result, response.Errormsg);
            if(response.Result == Result.Success)
            {//创建成功逻辑
            ...
            {
                this.OnCreate(response.Result, response.Errormsg);
            }
        }
